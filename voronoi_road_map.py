...
def find_nearest_road_point_index(x, y, sample_x, sample_y):
        min_dist = float('inf')
        min_index = None
        for i in range(len(sample_x)):
            d = abs(math.hypot(x - sample_x[i], y - sample_y[i]))
            if d < min_dist:
                min_dist = d
                min_index = i
...
class VoronoiRoadMapPlanner:
    ...
    def planning(self, sx, sy, gx, gy, ox, oy, robot_radius):
        ...
        sample_x, sample_y,x1,y1,x2,y2 = self.voronoi_sampling(sx, sy, gx, gy, ox, oy)
        ...
        # Connect start and goal points to the closest points on the roadmap
        rx, ry = DijkstraSearch(show_animation).search(x1, y1, x2, y2,
                                                       sample_x, sample_y,
                                                       road_map_info)
        # Combine paths
        # rfx = [sx].append(rx_start).appe
        # rx = [sx] + rx_start + rx + rx_goal + [gx]
        # ry = [sy] + ry_start + ry + ry_goal + [gy]

        return [sx]+rx+[gx], [sy]+ry+[gy]
    ...
    def voronoi_sampling(sx, sy, gx, gy, ox, oy):
        ...
        # Find the closest points on the roadmap to the start and goal points
        sx_nearest_idx = find_nearest_road_point_index(sx, sy, sample_x, sample_y)
        gx_nearest_idx = find_nearest_road_point_index(gx, gy, sample_x, sample_y)

        # If closest points not found, raise error
        if sx_nearest_idx is None or gx_nearest_idx is None:
            raise ValueError("Closest points to start or goal not found on roadmap.")
        x1,y1 = sample_x[sx_nearest_idx],sample_y[sx_nearest_idx]
        x2,y2 = sample_x[gx_nearest_idx],sample_y[gx_nearest_idx]
        sample_x.append(sx)
        sample_y.append(sy)
        sample_x.append(gx)
        sample_y.append(gy)

        return sample_x, sample_y,x1,y1,x2,y2
...
def main():
    ...
    x1 = float(input("Please enter start X-Coord: "))
    y1 = float(input("Please enter start Y-Coord: "))
    x2 = float(input("Please enter goal X-Coord: "))
    y2 = float(input("Please enter goal Y-Coord: "))
    sx = x1  # [m]
    sy = y1  # [m]
    gx = x2  # [m]
    gy = y2  # [m]
